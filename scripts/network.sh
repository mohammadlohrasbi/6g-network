...
generate_bundled_certs() {
  ...
  # TLS bundled (از ca.crt تولید شده توسط rca)
  echo "TLS bundled:"
  ...
  echo "اقدام بعدی: docker-compose down -v && docker-compose up -d"
  echo "لاگ peer را چک کن: docker logs peer0.org1.example.com | grep -i tls"
}
