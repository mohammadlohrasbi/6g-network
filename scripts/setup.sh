...
setup_network_with_fabric_ca_tls_nodeous_active() {
  ...
  # 8. تولید گواهی‌های نهایی با Enrollment CA (با root TLS CA برای --tls.certfiles)
  ...
  docker run --rm \
    ...
    /bin/bash -c "
      export FABRIC_CA_CLIENT_HOME=/tmp/fabric-ca-client
      export FABRIC_CA_CLIENT_TLS_INSECURE_SKIP_VERIFY=true  # برای دور زدن چک authority
      ...
      for i in {0..7}; do
        ...
        PORT=\$((7054 + (\$i + 1) * 100))
        ORG=\"org\$((i+1))\"
        ROOT_TLS_CERT=\"/crypto-config/peerOrganizations/\$ORG.example.com/tlsca/tlsca-\$ORG.\$ORG.example.com-cert.pem\"
