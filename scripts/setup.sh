...
package_and_install_chaincode() {
  ...
  for dir in "$CHAINCODE_DIR"/*/; do
    ...
    # این ۳ فایل دقیقاً باید باشه — بدون هیچ چیز اضافه!
    rm -f "$tar_file"
    (
      cd "$dir"
      tar -czf "$tar_file" \
    ...
    for i in {1..2}; do
      ...
      log "Chaincode $name روی Org${i} نصب شد" || true
    done
  ...
  success "تمام $total Chaincode با موفقیت نصب شدند (روش نهایی)"
}
